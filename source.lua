...
local Window = Libary:MakeWindow({
    Title = "Sigma Boy Hub | Blox Fruits ",
    SubTitle = "by: maverickk7_",
    LoadText = "Carregando Sigma Boy Hub",
    Flags = "SigmaBoyHub_BloxFruits"
})
...
local FarmTab = Window:MakeTab({ Title = "cu", Icon = "rbxassetid://15309138473" })

FarmTab:AddToggle({
    Name = "Auto Farm Level",
    Description = "Deixa o like",
    Default = false,
...
function UpdateRealFruitChams() 
	for i,v in pairs(game.Workspace.AppleSpawner:GetChildren()) do
		if v:IsA("Tool") then
			if RealFruitESP then 
				if not v.Handle:FindFirstChild('NameEsp'..Number) then
					local bill = Instance.new('Billboar